...
Nothing is failing any check now, and nothing regressed.
...
| | before | after |
|---|---:|---:|
| **conversion** (trip-planning intent) | 40% | 40% |
...
| latency, p95 | 6.06s | 5.29s |
| tokens per conversation | 4,519 | |
...
| check | before | after | |
|---|---:|---:|---|
| `grounded_in_tools` | 72% | 100% | fixed |
| `tool_output_integrity` | 89% | 100% | fixed |
| `scope_adherence` | 93% | 100% | fixed |
| `no_competitor` | 95% | 100% | fixed |
...
Nothing. Every case passes every check it's subject to.

That means every failure mode we wrote a test for is covered **on these
35 cases**, not that the agent is flawless. The honest next step is
growing the case list from real production traffic rather than our guesses.
